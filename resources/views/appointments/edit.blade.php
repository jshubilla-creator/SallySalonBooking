...
<div class="container mx-auto px-4 py-8">
    <div class="max-w-3xl mx-auto bg-white rounded-lg shadow-md p-6">
        ...
        <form action="{{ route('appointments.update', $appointment) }}" method="POST" id="appointmentForm">
                ...
                <select name="specialist_id" id="specialist_id" class="w-full border-gray-300 rounded-md shadow-sm" required>
                    ...
                    @foreach($specialists as $specialist)
                        <option value="{{ $specialist->id }}" 
                                {{ $appointment->specialist_id == $specialist->id ? 'selected' : '' }}>
                            {{ $specialist->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-6">
                <div class="flex items-center">
                    <input type="checkbox" name="is_home_service" id="is_home_service" 
                           ...
                           {{ $appointment->is_home_service ? 'checked' : '' }}>
            ...
            <div id="homeAddressSection" class="mb-6 {{ $appointment->is_home_service ? '' : 'hidden' }}">
                ...
                <textarea name="home_address" id="home_address" rows="3" 
                          class="w-full border-gray-300 rounded-md shadow-sm">{{ $appointment->home_address }}</textarea>
                <div id="addressValidationMessage" class="mt-2 text-sm">
                    @if($appointment->is_home_service && $appointment->distance_km)
                        <span class="text-green-600">
                            Address is valid. Distance: {{ number_format($appointment->distance_km, 2) }} km
                        </span>
                    @endif
                </div>
            </div>
            ...
            <div class="mb-6 bg-gray-50 p-4 rounded-md">
                ...
                <div class="space-y-2">
                    <div class="flex justify-between">
                        ...
                        <span id="servicePrice">₱{{ number_format($appointment->total_price, 2) }}</span>
                    </div>
                    <div id="homeServiceFeeSection" class="flex justify-between {{ $appointment->is_home_service ? '' : 'hidden' }}">
                        <span>Home Service Fee:</span>
                        <span id="homeServiceFee">₱{{ number_format($appointment->distance_fee ?? 0, 2) }}</span>
                    </div>
                    <div class="border-t pt-2 font-bold flex justify-between">
                        <span>Total:</span>
                        <span id="totalPrice">₱{{ number_format($appointment->grand_total, 2) }}</span>
                    </div>
                </div>
            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    let debounceTimer;
    let currentServicePrice = parseFloat(serviceSelect.options[serviceSelect.selectedIndex].dataset.price) || 0;
    let currentHomeServiceFee = {{ $appointment->distance_fee ?? 0 }};
    ...
    isHomeService.addEventListener('change', function() {
        ...
        if (!this.checked) {
            ...
            updatePriceDisplay();
        } else if (homeAddress.value.trim()) {
            validateAddress(homeAddress.value);
        }
    });
    ...
    function validateAddress(address) {
        if (!isHomeService.checked) return;
