<x-admin-layout>
                                ...
                                @forelse($specialists as $specialist)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                            <div class="flex flex-col sm:flex-row gap-2">
                                                <button type="button"
                                                        onclick="openSpecialistModal({{ $specialist->id }}, '{{ $specialist->name }}', '{{ $specialist->email }}', '{{ $specialist->phone }}', '{{ $specialist->specialization }}', '{{ $specialist->experience_years }}', '{{ $specialist->bio }}', {{ json_encode($specialist->working_hours ?? []) }}, {{ json_encode($specialist->services->pluck('name')->toArray()) }}, {{ $specialist->is_available ? 'true' : 'false' }})"
    ...
    <div id="specialist-modal" class="fixed inset-0 bg-black bg-opacity-50 z-50 hidden flex items-center justify-center p-4">
        <div class="bg-blue-100 shadow-lg max-w-2xl w-full max-h-[90vh] flex flex-col rounded-lg overflow-hidden">
            <div class="bg-blue-100 rounded-lg px-6 py-4 border-b border-gray-200 flex justify-between items-center flex-shrink-0">
