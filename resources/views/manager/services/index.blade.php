<x-manager-layout>
    ...
    <div class="mb-8">
        <div class="flex justify-between items-center">
            <div>
                ...
                <p class="mt-2 text-lg text-gray-600">Manage salon services and pricing</p>
            </div>
            <div class="flex space-x-3">
                <a href="{{ route('manager.services.create') }}"
                   ...
                   <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2zm7-10v6m3-3H9"/>
                    </svg>
                    Add New Service
                </a>
            </div>
    ...
    <div class="bg-gradient-to-br from-pink-100 via-purple-50 to-indigo-100 shadow rounded-lg p-6 mb-6">
        <form method="GET" action="{{ route('manager.services.index') }}" class="space-y-4">
            <!-- Search Bar -->
            <div>
                            ...
                            <tr class="hover:bg-gray-50">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center">
                                        <div class="flex-shrink-0 h-10 w-10">
                                            @if($service->image)
                                                <img class="h-10 w-10 rounded-full object-cover" src="{{ $service->image }}" alt="{{ $service->name }}">
                                            @else
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <div class="flex flex-col sm:flex-row gap-2">
                                        ...
                                        <button type="button"
                                                onclick="showConfirmation('Delete Service', 'Are you sure you want to delete this service? This action cannot be undone.', function() { document.getElementById('delete-form-{{ $service->id }}').submit(); })"
                                        ...
                                        <form id="delete-form-{{ $service->id }}" method="POST" action="{{ route('manager.services.destroy', $service) }}" class="hidden">
                                            @csrf
                                            @method('DELETE')
                                        </form>
                                    </div>
