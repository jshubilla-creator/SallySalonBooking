<x-manager-layout>
    ...
    <div class="mb-8">
        <div class="flex justify-between items-center">
            <div>
                ...
                <p class="mt-2 text-lg text-gray-600">Update {{ $service->name }}</p>
            </div>
    ...
    <div class="max-w-2xl">
        <form action="{{ route('manager.services.update', $service) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="description"
                                  id="description"
                                  rows="3"
                                  class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-green-500 @error('description') border-red-500 @enderror">{{ old('description', $service->description) }}</textarea>
                        @error('description')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <select name="category"
                            ...
                            <option value="Nail" {{ old('category', $service->category) === 'Nail' ? 'selected' : '' }}>Nail</option>
